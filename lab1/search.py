...
class SearchNode:
    ...
    def backtrack(self):
        ...
        curr_node = self
        moves.append(curr_node.transition)
        curr_node = node.parent
        moves += curr_node.backtrack()
        return moves
...
def depthFirstSearch(problem):
    ...
    while(not stack.isEmpty()):
        curr_node_stack = stack.pop()

        visited.add(curr_node_stack.position)

        if problem.isGoalState(curr_node_stack.position):
            list_direction = curr_node_stack.backtrack()
            list_direction.reverse()
            return list_direction

        for x in problem.getSuccessors(curr_node_stack.position):
            if not x[0] in visited:
                succ_node = SearchNode(x[0])
                succ_node.transition = x[1]
                succ_node.parent = curr_node_stack
                stack.push(succ_node)
                #visited.add(succ_node.position)

            else:
                succ_node = SearchNode(x[0])
                succ_node.transition = x[1]
                succ_node.parent = curr_node_stack
                if problem.isGoalState(succ_node.position):
                    list_direction = succ_node.backtrack()
                    list_direction.reverse()
                    return list_direction
                #visited.add(succ_node.position)
    util.raiseNotDefined()
...
def uniformCostSearch(problem):
    ...
    problem.getStartState()
    start_node = SearchNode(problem.getStartState())
    p_queue.push(start_node, start_node.cost)
    visited.add(start_node.position)

    while (not p_queue.isEmpty()):
        curr_node_stack = p_queue.pop()
        visited.add(curr_node_stack.position)
        ...
        for x in problem.getSuccessors(curr_node_stack.position):
            if not x[0] in visited:
                ...
                succ_node.cost = x[2]
                succ_node.parent = curr_node_stack
                succ_node.cost += curr_node_stack.cost
                visited.add(succ_node.position)
                p_queue.push(succ_node, succ_node.cost)
    ...
    while (not p_queue.isEmpty()):
        ...
        for x in problem.getSuccessors(curr_node_stack.position):
            if not x[0] in visited:
                ...
                succ_node.parent = curr_node_stack
                succ_node.cost += curr_node_stack.cost
                priority = succ_node.cost + heuristic(succ_node.position, problem)
                visited.add(succ_node.position)
                p_queue.push(succ_node, priority)
